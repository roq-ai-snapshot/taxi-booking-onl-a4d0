...
import { usePriceCount, useVehicleCount, useCityCount } from 'lib/roq';

import { CityListPage as ShowCaseList } from 'pages/cities';

function DashboardPage() {
  const { session } = useSession();

  const priceStat = usePriceCount();

  const vehicleStat = useVehicleCount();

  const cityStat = useCityCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Prices',
      result: priceStat?.data,
      isLoading: priceStat.isLoading,
    ...
    {
      title: 'Cities',
      result: cityStat?.data,
      isLoading: cityStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`cities`} passHref legacyBehavior>
                  <Button
