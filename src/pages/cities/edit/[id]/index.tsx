...
import { useRoqClient, useCityFindFirst } from 'lib/roq';
...
function CityEditPage() {
  ...
  const { data, error, isLoading, mutate } = useCityFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: CityInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.city.update({
        data: values as RoqTypes.city,
        where: {
          id,
        },
      });
