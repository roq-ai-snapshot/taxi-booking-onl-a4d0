...
export function BookingListPage(props: BookingListPageProps) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_booking_customer_idTouser',
          header: 'User Booking Customer Id Touser',
          accessorKey: 'user_booking_customer_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_booking_customer_idTouser?.id}`}
            >
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_booking_driver_idTouser',
          header: 'User Booking Driver Id Touser',
          accessorKey: 'user_booking_driver_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_booking_driver_idTouser?.id}`}
            >
              {record.user_booking_driver_idTouser?.email}
            </Link>
          ),
        }
      : null,
    ...
    hasAccess('city', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'city_booking_start_city_idTocity',
          header: 'City Booking Start City Id Tocity',
          accessorKey: 'city_booking_start_city_idTocity',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/cities/view/${record.city_booking_start_city_idTocity?.id}`}
            >
              {record.city_booking_start_city_idTocity?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('city', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'city_booking_end_city_idTocity',
          header: 'City Booking End City Id Tocity',
          accessorKey: 'city_booking_end_city_idTocity',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/cities/view/${record.city_booking_end_city_idTocity?.id}`}
            >
              {record.city_booking_end_city_idTocity?.name}
            </Link>
          ),
        }
      : null,
    hasAccess('price', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'price',
          header: 'Price',
          accessorKey: 'price',
          cell: ({ row: { original: record } }: any) => record?._count?.price,
        }
      : null,
