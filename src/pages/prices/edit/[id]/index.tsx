...
import { useRoqClient, usePriceFindFirst } from 'lib/roq';
...
function PriceEditPage() {
  ...
  const { data, error, isLoading, mutate } = usePriceFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: PriceInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.price.update({
        data: values as RoqTypes.price,
        where: {
          id,
        },
      });
